...
const submitOTP = handleSubmit(async (values) => {
  const tokenObject = await verifyOTP(
    "accounts/onboard/verify/",
    values,
    sessionToken.value
  );
  ...
  <form @submit="onSubmit">
    <v-sheet width="400" class="mx-auto">
      <v-card color="transparent" class="text-center py-6">
        ...
        <v-btn type="submit" color="green-lighten-2"> login </v-btn>
        <div v-if="logged">
          <v-otp-input v-model="otp" v-bind="otpAttrs" name="otp" focus-all>
          </v-otp-input>

          <span>{{ errors.otp }}</span>

          <v-btn @click="submitOTP"> Enter OTP </v-btn>
        </div>
      </v-card>
    </v-sheet>
  </form>
